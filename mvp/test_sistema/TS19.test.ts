...
describe("Richiesta analisi secondo il cappello nero", () => {
  it("acquisisce la richiesta e avvia l'elaborazione", () => {
    ...
    void model.requestAIOperation(HAT_ANALYSIS, "testo selezionato", { hat_type: HAT_BLACK });

    expect(model.getAIState()).toBeInstanceOf(ProcessingState);
  });
});

describe("Generazione analisi secondo il cappello nero", () => {
  it("inoltra la richiesta con hat_type 'black'", () => {
    const aiService = makeMockAIService();
    const deferred = createDeferred<Proposal>();
    (aiService.requestOperation as any).mockReturnValue(deferred.promise);
...
describe("Visualizzazione elaborazione analisi secondo il cappello nero", () => {
  it("notifica gli observer e mantiene lo stato Processing", async () => {
    ...
    const requestPromise = model.requestAIOperation(HAT_ANALYSIS, "testo", { hat_type: HAT_BLACK });

    expect(observer.update).toHaveBeenCalledTimes(1);
    expect(model.getAIState()).toBeInstanceOf(ProcessingState);

    deferred.resolve(new Proposal("analisi cappello nero", HAT_ANALYSIS));
    await requestPromise;
  });
});

describe("Interruzione elaborazione analisi secondo il cappello nero", () => {
  it("torna allo stato Idle quando l'utente interrompe", () => {
    ...
    void model.requestAIOperation(HAT_ANALYSIS, "testo", { hat_type: HAT_BLACK });
    expect(model.getAIState()).toBeInstanceOf(ProcessingState);
...
describe("Visualizzazione proposta analisi secondo il cappello nero", () => {
  it("porta lo stato a ProposalReady con la proposta generata", async () => {
    const aiService = makeMockAIService();
    const proposal = new Proposal("Analisi dei rischi e criticità.", HAT_ANALYSIS);
    ...
    await model.requestAIOperation(HAT_ANALYSIS, "testo", { hat_type: HAT_BLACK });
    ...
    expect((state as ProposalReadyState).proposal.content).toBe("Analisi dei rischi e criticità.");
    expect(observer.update).toHaveBeenCalledTimes(2);
  });
});

describe("Accettazione proposta analisi secondo il cappello nero", () => {
  it("sostituisce il testo selezionato con l'analisi proposta", async () => {
    const aiService = makeMockAIService();
    const proposal = new Proposal("ANALISI_CAPPELLO_NERO", HAT_ANALYSIS);
    (aiService.requestOperation as any).mockResolvedValue(proposal);
    ...
    expect(noteModel.getContent()).toBe("Questo è il ANALISI_CAPPELLO_NERO");
    expect(model.getAIState()).toBeInstanceOf(IdleState);
  });
});

describe("Rifiuto proposta analisi secondo il cappello nero", () => {
  it("torna allo stato Idle senza alterare la nota", async () => {
    const aiService = makeMockAIService();
    const proposal = new Proposal("analisi cappello nero", HAT_ANALYSIS);
    (aiService.requestOperation as any).mockResolvedValue(proposal);
...
describe("Rigenerazione proposta analisi secondo il cappello nero", () => {
...
describe("Errore generazione analisi secondo il cappello nero", () => {
