...
describe("Richiesta analisi secondo il cappello verde", () => {
...
describe("Generazione analisi secondo il cappello verde", () => {
  it("inoltra la richiesta con hat_type 'green'", () => {
    const aiService = makeMockAIService();
    const deferred = createDeferred<Proposal>();
    (aiService.requestOperation as any).mockReturnValue(deferred.promise);
...
describe("Visualizzazione elaborazione analisi secondo il cappello verde", () => {
  it("notifica gli observer e mantiene lo stato Processing", async () => {
    ...
    deferred.resolve(new Proposal("analisi cappello verde", HAT_ANALYSIS));
    await requestPromise;
  });
});

describe("Interruzione elaborazione analisi secondo il cappello verde", () => {
...
describe("Accettazione proposta analisi secondo il cappello verde", () => {
  it("sostituisce il testo selezionato con l'analisi proposta", async () => {
    const aiService = makeMockAIService();
    const proposal = new Proposal("ANALISI_CAPPELLO_VERDE", HAT_ANALYSIS);
    (aiService.requestOperation as any).mockResolvedValue(proposal);
    ...
    expect(noteModel.getContent()).toBe("Questo è il ANALISI_CAPPELLO_VERDE");
    expect(model.getAIState()).toBeInstanceOf(IdleState);
  });
});

describe("Rifiuto proposta analisi secondo il cappello verde", () => {
  it("torna allo stato Idle senza alterare la nota", async () => {
    const aiService = makeMockAIService();
    const proposal = new Proposal("analisi cappello verde", HAT_ANALYSIS);
    (aiService.requestOperation as any).mockResolvedValue(proposal);
...
describe("Rigenerazione proposta analisi secondo il cappello verde", () => {
