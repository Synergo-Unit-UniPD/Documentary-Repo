...
describe("Richiesta analisi secondo il cappello rosso", () => {
  it("acquisisce la richiesta e avvia l'elaborazione", () => {
    ...
    void model.requestAIOperation(HAT_ANALYSIS, "testo selezionato", { hat_type: HAT_RED });

    expect(model.getAIState()).toBeInstanceOf(ProcessingState);
  });
});

describe("Generazione analisi secondo il cappello rosso", () => {
  it("inoltra la richiesta con hat_type 'red'", () => {
    const aiService = makeMockAIService();
    const deferred = createDeferred<Proposal>();
    (aiService.requestOperation as any).mockReturnValue(deferred.promise);
...
describe("Visualizzazione elaborazione analisi secondo il cappello rosso", () => {
  it("notifica gli observer e mantiene lo stato Processing", async () => {
    ...
    const requestPromise = model.requestAIOperation(HAT_ANALYSIS, "testo", { hat_type: HAT_RED });

    expect(observer.update).toHaveBeenCalledTimes(1);
    expect(model.getAIState()).toBeInstanceOf(ProcessingState);

    deferred.resolve(new Proposal("analisi cappello rosso", HAT_ANALYSIS));
    await requestPromise;
  });
});

describe("Interruzione elaborazione analisi secondo il cappello rosso", () => {
  it("torna allo stato Idle quando l'utente interrompe", () => {
    ...
    void model.requestAIOperation(HAT_ANALYSIS, "testo", { hat_type: HAT_RED });
    expect(model.getAIState()).toBeInstanceOf(ProcessingState);
...
describe("Visualizzazione proposta analisi secondo il cappello rosso", () => {
  it("porta lo stato a ProposalReady con la proposta generata", async () => {
    const aiService = makeMockAIService();
    const proposal = new Proposal("Analisi emotiva del testo.", HAT_ANALYSIS);
    ...
    await model.requestAIOperation(HAT_ANALYSIS, "testo", { hat_type: HAT_RED });
    ...
    expect((state as ProposalReadyState).proposal.content).toBe("Analisi emotiva del testo.");
    expect(observer.update).toHaveBeenCalledTimes(2);
  });
});

describe("Accettazione proposta analisi secondo il cappello rosso", () => {
  it("sostituisce il testo selezionato con l'analisi proposta", async () => {
    const aiService = makeMockAIService();
    const proposal = new Proposal("ANALISI_CAPPELLO_ROSSO", HAT_ANALYSIS);
    (aiService.requestOperation as any).mockResolvedValue(proposal);
    ...
    expect(noteModel.getContent()).toBe("Questo è il ANALISI_CAPPELLO_ROSSO");
    expect(model.getAIState()).toBeInstanceOf(IdleState);
  });
});

describe("Rifiuto proposta analisi secondo il cappello rosso", () => {
  it("torna allo stato Idle senza alterare la nota", async () => {
    const aiService = makeMockAIService();
    const proposal = new Proposal("analisi cappello rosso", HAT_ANALYSIS);
    (aiService.requestOperation as any).mockResolvedValue(proposal);
...
describe("Rigenerazione proposta analisi secondo il cappello rosso", () => {
...
describe("Errore generazione analisi secondo il cappello rosso", () => {
