...
describe("Richiesta analisi secondo il cappello blu", () => {
  it("acquisisce la richiesta e avvia l'elaborazione", () => {
    ...
    void model.requestAIOperation(HAT_ANALYSIS, "testo selezionato", { hat_type: HAT_BLUE });

    expect(model.getAIState()).toBeInstanceOf(ProcessingState);
  });
});

describe("Generazione analisi secondo il cappello blu", () => {
  it("inoltra la richiesta con hat_type 'blue'", () => {
    const aiService = makeMockAIService();
    const deferred = createDeferred<Proposal>();
    (aiService.requestOperation as any).mockReturnValue(deferred.promise);
...
describe("Visualizzazione elaborazione analisi secondo il cappello blu", () => {
  it("notifica gli observer e mantiene lo stato Processing", async () => {
    ...
    const requestPromise = model.requestAIOperation(HAT_ANALYSIS, "testo", { hat_type: HAT_BLUE });

    expect(observer.update).toHaveBeenCalledTimes(1);
    expect(model.getAIState()).toBeInstanceOf(ProcessingState);

    deferred.resolve(new Proposal("analisi cappello blu", HAT_ANALYSIS));
    await requestPromise;
  });
});

describe("Interruzione elaborazione analisi secondo il cappello blu", () => {
  it("torna allo stato Idle quando l'utente interrompe", () => {
    ...
    void model.requestAIOperation(HAT_ANALYSIS, "testo", { hat_type: HAT_BLUE });
    expect(model.getAIState()).toBeInstanceOf(ProcessingState);
...
describe("Visualizzazione proposta analisi secondo il cappello blu", () => {
  it("porta lo stato a ProposalReady con la proposta generata", async () => {
    const aiService = makeMockAIService();
    const proposal = new Proposal("Analisi della struttura e coerenza logica.", HAT_ANALYSIS);
    ...
    await model.requestAIOperation(HAT_ANALYSIS, "testo", { hat_type: HAT_BLUE });
    ...
    expect((state as ProposalReadyState).proposal.content).toBe("Analisi della struttura e coerenza logica.");
    expect(observer.update).toHaveBeenCalledTimes(2);
  });
});

describe("Accettazione proposta analisi secondo il cappello blu", () => {
  it("sostituisce il testo selezionato con l'analisi proposta", async () => {
    const aiService = makeMockAIService();
    const proposal = new Proposal("ANALISI_CAPPELLO_BLU", HAT_ANALYSIS);
    (aiService.requestOperation as any).mockResolvedValue(proposal);
    ...
    expect(noteModel.getContent()).toBe("Questo è il ANALISI_CAPPELLO_BLU");
    expect(model.getAIState()).toBeInstanceOf(IdleState);
  });
});

describe("Rifiuto proposta analisi secondo il cappello blu", () => {
  it("torna allo stato Idle senza alterare la nota", async () => {
    const aiService = makeMockAIService();
    const proposal = new Proposal("analisi cappello blu", HAT_ANALYSIS);
    (aiService.requestOperation as any).mockResolvedValue(proposal);
...
describe("Rigenerazione proposta analisi secondo il cappello blu", () => {
...
describe("Errore generazione analisi secondo il cappello blu", () => {
