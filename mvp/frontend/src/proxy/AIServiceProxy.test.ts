...
describe('AIServiceProxy', () => {
    ...
    it('dovrebbe recuperare la lista delle operazioni disponibili (GET api/ai-operations)', async () => {
        ...
        expect(globalThis.fetch).toHaveBeenCalledWith('http://localhost:8000/api/ai-operations', expect.objectContaining({ method: 'GET' }));
    ...
    it('dovrebbe inviare una requestOperation e restituire una Proposal', async () => {
        ...
        expect(globalThis.fetch).toHaveBeenCalledWith('http://localhost:8000/api/ai-operations', expect.objectContaining({
            method: 'POST',
