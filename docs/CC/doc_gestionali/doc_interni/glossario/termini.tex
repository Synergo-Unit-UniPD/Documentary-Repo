...
\begin{itemize}[leftmargin=*]
    \item \textbf{Semantic Versioning:} Convenzione di versionamento adottata dal
          ...
          semantiche.
    \item \textbf{Struttura Documentale:} Organizzazione interna dei file e delle
          cartelle che compongono un documento, definita dal template condiviso del
          gruppo. Comprende: cartella principale nominata con il nome del documento,
          file \texttt{.tex} separati per sezioni, cartella \texttt{shared/} per
          loghi e configurazioni comuni.
\end{itemize}
...
\begin{itemize}[leftmargin=*]
    ...
    \item \textbf{.txt:} Formato di file di testo semplice privo di formattazione.
          Nel contesto del gruppo, il file \texttt{.txt} contenuto nella cartella
          \texttt{python\_glossary\_automation/} elenca i termini del Glossario
          utilizzati dallo script Python per applicare automaticamente il pedice
          G\textsubscript{G} alla prima occorrenza di ciascun termine nei documenti
          \LaTeX{}.
\end{itemize}
