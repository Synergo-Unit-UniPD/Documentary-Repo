...
Si applica a due livelli distinti e si espande al crescere del progetto: la
Sez.~\ref{sec:dod-doc} è attiva fin dal primo sprint, mentre, la Sez.~\ref{sec:dod-codice} dall'inizio dello sviluppo del PoC.

% ── 4.1 Livello task — fase documentale ─────────────────────────────────────
\subsection{Livello task: Fase documentale}
\label{sec:dod-doc}

\textbf{Attiva da:} Sprint 1. 

\textbf{Applicabile a:} tutti i task documentali.
...
\begin{itemize}
    ...
    \item Il task è linkato nel verbale della riunione dal Redattore.
    \item È stato creato un branch da \texttt{development} direttamente dalla issue,
          con naming: \\
          \texttt{<id>-<gestionale|tecnico>-<esterno|interno>-<nomeDoc>}
...
\begin{itemize}
    \item Il documento rispetta il template e la struttura concordata nelle Norme di
          Progetto.
...
\subsubsection*{Commit e Pull Request}
\begin{itemize}
    ...
    \item La PR è aperta \textbf{entro domenica} per dare tempo al Verificatore di
...
\begin{itemize}
    \item Il Verificatore assegnato ha revisionato il
          documento.
    \item Tutti i commenti della review sono stati risolti o discussi prima del merge.
    \item È il Redattore a fare il merge, non il Verificatore.
\end{itemize}

\subsubsection*{Chiusura}
\begin{itemize}
    \item Merge effettuato su \texttt{main}.
...
\textbf{Attiva da:} Sprint (da definire). 

\textbf{Applicabile a:} tutti i task di sviluppo software.

\medskip
\textbf{[Da compilare]}
\textit{Questa sezione verrà completata quando il team avvierà lo sviluppo del
Proof of Concept. I criteri includeranno almeno:}
\begin{itemize}
    ...
    \item linting superato secondo la configurazione del repository;
