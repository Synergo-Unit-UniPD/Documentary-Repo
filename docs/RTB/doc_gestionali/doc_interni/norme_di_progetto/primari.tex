...
In conformità allo standard ISO/IEC~12207:1995\textsubscript{G}, i processi primari
...
\begin{itemize}
    ...
    particolare riferimento, in fase RTB\textsubscript{G}, all'analisi dei requisiti,
    all'analisi dello stato dell'arte, alla progettazione preliminare e alla
    definizione del Proof of Concept\textsubscript{G}.
\end{itemize}

Le attività di codifica estesa, testing automatico, integrazione continua e
rilascio software verranno progressivamente normate nella fase PB\textsubscript{G},
...
\begin{itemize}
    \item il \textbf{docente} ricopre il ruolo di committente rispetto alle forniture,
    alle scadenze, ai vincoli regolamentari e alla valutazione delle baseline;

    \item la \textbf{proponente}, Zucchetti\textsubscript{G} S.p.A., ricopre il ruolo
...
\begin{itemize}
    ...
    \item la consegna della documentazione tramite repository pubblico e sito web.
\end{itemize}
...
\begin{itemize}
      ...
      e pubblicazione degli artefatti documentali;

      \item \textbf{GitHub Pages}: utilizzato per rendere consultabile la documentazione
      prodotta;

      \item \textbf{GitHub Projects}: utilizzato per il tracciamento delle attività,
      delle milestone RTB e PB e dello stato di avanzamento;
...
Durante la fase di candidatura\textsubscript{G} (CC), il gruppo ha prodotto e
...
\begin{xltabular}{\textwidth}{>{\hsize=.32\hsize\raggedright\arraybackslash}X
    ...
    \doclink{https://github.com/Synergo-Unit-UniPD/Documentary-Repo/blob/main/docs/CC/doc_gestionali/doc_esterni/lettera_presentazione/lettera_presentazione.pdf}
    ...
    \doclink{https://github.com/Synergo-Unit-UniPD/Documentary-Repo/blob/main/docs/CC/doc_gestionali/doc_esterni/valutazione_capitolati/valutazione_capitolati.pdf}
    ...
    \doclink{https://github.com/Synergo-Unit-UniPD/Documentary-Repo/blob/main/docs/CC/doc_gestionali/doc_esterni/preventivo_costi_assunzione_impegni/preventivo_costi_e_assunzione_impegni.pdf}
...
\begin{xltabular}{\textwidth}{
    ...
    partecipazione alla revisione e rimanda alla documentazione disponibile nel
    repository pubblico. \\

    \doclink{https://synergo-unit-unipd.github.io/Documentary-Repo/docs/RTB/doc_gestionali/doc_esterni/piano_di_progetto/piano_di_progetto.pdf}
    {Piano di Progetto}
    & Amministratore
    & Esterno
    & Documento che descrive pianificazione, consuntivazione, gestione dei rischi,
    rotazione dei ruoli, avanzamento degli sprint e controllo dei costi. \\

    \doclink{https://synergo-unit-unipd.github.io/Documentary-Repo/docs/RTB/doc_gestionali/doc_esterni/piano_di_qualifica/piano_di_qualifica.pdf}
    {Piano di Qualifica}
    & Amministratore
    & Esterno
    ...
    \doclink{https://synergo-unit-unipd.github.io/Documentary-Repo/docs/RTB/doc_tecnici/doc_esterni/analisi_dei_requisiti/analisi_dei_requisiti.pdf}
    {Analisi dei Requisiti}
    & Analista
    & Esterno
    & Documento tecnico che formalizza casi d'uso, requisiti, fonti e tracciabilità
    tra esigenze espresse, funzionalità attese e vincoli del prodotto. \\

    Norme di Progetto
    & Amministratore
    & Interno
    & Documento che definisce il way of working\textsubscript{G} del gruppo. In RTB viene
    aggiornato alla versione 2.x.x per includere i processi attivi dopo la candidatura. \\

    \doclink{https://synergo-unit-unipd.github.io/Documentary-Repo/docs/RTB/doc_gestionali/doc_interni/glossario/glossario.pdf}
    {Glossario}
    & Amministratore
    & Interno
    ...
    \doclink{https://synergo-unit-unipd.github.io/Documentary-Repo/docs/RTB/doc_gestionali/doc_interni/analisi_stato_arte/analisi_stato_arte.pdf}
...
\begin{itemize}
    ...
    \item Zucchetti\textsubscript{G} S.p.A.;
...
di presentazione della Requirements \& Technology Baseline\textsubscript{G} e, ove necessario, nei Diari
di Bordo.

Gli Architecture Decision Records (ADR)\textsubscript{G} non sono ancora considerati
...
\begin{itemize}
    ...
    \item \textbf{Input}: Piano di Progetto, attività pianificate, issue assegnate.
...
\begin{itemize}
    \item \textbf{Ruoli}: Responsabile, Verificatore, ruoli coinvolti negli artefatti.
...
il distant writing e i requisiti prestazionali.
...
\begin{itemize}
    ...
    \item definizione del Proof of Concept.
\end{itemize}

La realizzazione del PoC può prevedere attività di codifica limitate e finalizzate
alla sola validazione tecnica delle scelte effettuate; tali attività non costituiscono
ancora il processo completo di sviluppo software previsto per la fase PB.

Le attività di codifica estesa, testing automatico, integrazione continua e rilascio
software saranno normate in modo completo nella fase PB.

% ------------------------------------------------

\subsubsection{Modello di sviluppo}

Il gruppo adotta un modello organizzativo incrementale ispirato a Scrum\textsubscript{G},
basato su sprint settimanali con cadenza da martedì a martedì.

Il workflow prevede:

\begin{itemize}
    \item sprint retrospective dello sprint precedente;
    \item sprint review dello sprint precedente;
    \item sprint planning dello sprint corrente;
    \item daily stand-up asincrono;
...
\begin{itemize}
    \item \textbf{Gestione editor e Markdown}: scrittura, formattazione e rendering;
...
\begin{itemize}
      ...
      mantenuti tramite LucidChart/LucidSpark o strumenti equivalenti condivisi dal
      gruppo;
...
\begin{itemize}
    ...
    \item \textbf{Backend}: Python con FastAPI\textsubscript{G};
    \item \textbf{Database}: PostgreSQL\textsubscript{G};
    \item \textbf{Containerizzazione}: Docker\textsubscript{G}.
\end{itemize}

Le scelte tecnologiche rimangono soggette a confronto con la proponente e potranno
essere aggiornate in seguito ai risultati del Proof of Concept.
...
\begin{itemize}
    ...
    \item valutare le interazioni tra frontend, backend, database e servizi AI;
...
Il Proof of Concept ha lo scopo di validare la fattibilità tecnica delle principali
funzionalità individuate dal gruppo per la Requirements \& Technology Baseline\textsubscript{G}.

Il PoC costituisce una baseline tecnologica e non architetturale: esso ha valore
dimostrativo e può essere rivisto o scartato nella successiva fase PB, qualora le
evidenze raccolte lo rendano opportuno.

\paragraph{Obiettivi}

Il PoC dovrà verificare:

\begin{itemize}
    \item integrazione con LLM;
    \item importazione e salvataggio di note Markdown;
    \item apertura iniziale di una nota esistente;
    \item visualizzazione split editor;
    \item applicazione di uno stile;
    \item interazione con LLM tramite bottone o popup;
    \item accettazione, rifiuto e rigenerazione di proposte;
    \item proposta contestuale su selezione di testo;
    \item distant writing tramite inserimento di prompt.
\end{itemize}
...
\begin{itemize}
    \item dimostrare l'integrazione minima tra frontend, backend, database e servizio LLM;
    \item verificare almeno un flusso completo di gestione di una nota Markdown;
    \item mostrare l'interazione utente con una proposta generata dal modello;
    \item raccogliere evidenze utili alla progettazione di dettaglio;
