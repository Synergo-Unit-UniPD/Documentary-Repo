...
\begin{itemize}
    ...
    retrospettive, pianificazione e assegnazione dei task.
\end{itemize}
...
\begin{itemize}
    ...
    gli stakeholder esterni. Sono redatti dall'Amministratore e
    ...
    periodico con il committente. \`E redatto tramite template Canva e non segue
...
\begin{itemize}
    ...
    automaticamente il pedice  alla prima occorrenza dei termini
...
\begin{itemize}
    ...
    Title Case italianizzato, con maiuscola sulle parole semanticamente
...
\begin{itemize}
    ...
    \item il pedice  sia stato applicato correttamente;
...
\begin{center}
    \texttt{doc-<categoria>-<uso>-<nome-documento>}
\end{center}
...
\begin{itemize}
    \item \texttt{doc-gestionale-interno-norme-di-progetto};
    \item \texttt{doc-gestionale-esterno-piano-di-qualifica};
    \item \texttt{doc-tecnico-esterno-analisi-dei-requisiti}.
\end{itemize}
...
\begin{itemize}
    \item \textbf{feature}: nuova funzionalità del prodotto o del PoC;
    \item \textbf{bug}: comportamento non corretto rispetto all'atteso;
    \item \textbf{refactor}: miglioramento interno senza modifica del comportamento
    osservabile;
...
\begin{center}
    \texttt{tipo-posizione-descrizione-breve}
\end{center}

La descrizione breve deve essere scritta in minuscolo, senza accenti e con parole
separate da trattini.

\textit{Esempi}:

\begin{itemize}
    \item \texttt{feature-frontend-css};
    \item \texttt{bug-frontend-css};
    \item \texttt{refactor-infra-aggiornamento-token};
    \item \texttt{test-backend-test-chiamata-ai}.
\end{itemize}
...
Il gruppo adotta un modello \textit{trunk-based development}.
...
\begin{center}
    \texttt{<id>-doc-<categoria>-<uso>-<nome-documento>}
\end{center}
...
\begin{itemize}
    ...
    \item \texttt{categoria} indica la natura del documento e può assumere i valori
    \texttt{gestionale} o \texttt{tecnico};
    \item \texttt{uso} indica la destinazione del documento e può assumere i valori
    \texttt{interno} o \texttt{esterno};
    \item \texttt{nome-documento} identifica il documento o l'artefatto documentale
    modificato.
\end{itemize}
...
\begin{itemize}
    \item \texttt{146-doc-gestionale-interno-norme-di-progetto};
    \item \texttt{147-doc-gestionale-esterno-piano-di-qualifica};
    \item \texttt{148-doc-tecnico-esterno-analisi-dei-requisiti}.
\end{itemize}
...
\begin{center}
    \texttt{<id>-<tipo>-<posizione>-<descrizione-breve>}
\end{center}
...
\begin{itemize}
    \item \texttt{142-feature-frontend-css};
    \item \texttt{143-bug-frontend-css};
    \item \texttt{144-refactor-infra-aggiornamento-token};
    \item \texttt{145-test-backend-test-chiamata-ai}.
\end{itemize}
...
Ogni commit deve seguire una convenzione ispirata ai Conventional Commits,
...
Per i commit documentali devono essere utilizzati i seguenti scope:
...
\begin{itemize}
    ...
    \item presenza del pedice  dove necessario;
...
\begin{itemize}
    ...
    \item aggiornamento del README se la modifica cambia la procedura di avvio;
    \item descrizione della prova effettuata per modifiche backend o API;
...
\begin{itemize}
    \item il task è stato creato tramite Google Form;
...
\begin{itemize}
    ...
    \item il pedice  è stato applicato correttamente.
\end{itemize}
...
Il gruppo adotta il ciclo PDCA come riferimento per il miglioramento
continuo:
...
Il Piano di Qualifica definisce le metriche, le soglie e il cruscotto di qualità
...
riportati nel \link{https://synergo-unit-unipd.github.io/Second-Brain/docs/RTB/doc_gestionali/doc_esterni/piano_di_qualifica/piano_di_qualifica.pdf}{Piano di Qualifica}.
...
Le metriche vengono aggiornate ogni due sprint, integrando i dati raccolti negli
sprint conclusi e aggiornando le rappresentazioni tabellari e grafiche presenti
...
\begin{xltabular}{\textwidth}{
    ...
    MPC07
    & Review Coverage
    & Misurare la percentuale di documenti o file verificati da un membro diverso dall'autore. \\
...
\begin{xltabular}{\textwidth}{
    ...
    MPD06
    & Tempo Iniziale di Caricamento del Programma
