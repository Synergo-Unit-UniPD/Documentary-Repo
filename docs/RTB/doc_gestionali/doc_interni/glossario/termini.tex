...
\begin{itemize}[leftmargin=*]
    ...
    \item \textbf{Distant Writing:} Modalità di interazione in cui l'utente delega interamente la stesura di un testo a un sistema di Intelligenza Artificiale, fornendo esclusivamente le direttive iniziali (prompt).
...
\begin{itemize}[leftmargin=*]
    \item \textbf{LLM (Large Language Model):} Modello avanzato di intelligenza
      ...
      \item \textbf{Lucidchart:} Applicazione web utilizzata dal gruppo per la creazione collaborativa di diagrammi, in particolare per la modellazione UML.
...
\begin{itemize}[leftmargin=*]
      ...
      \item \textbf{Split View:} Modalità di visualizzazione dell'interfaccia utente che suddivide lo schermo in due o più sezioni, permettendo di osservare contemporaneamente l'editor di testo e la sua anteprima formattata.
\end{itemize}
