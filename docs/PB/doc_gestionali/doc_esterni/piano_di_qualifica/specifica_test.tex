...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|X|c|c|c|}
        ...
        \hline
        API\_business\_layer/api -- router\textsubscript{G} e schemi & 2 & 11 & S \\
        \hline
        API\_business\_layer/di -- Dependency Injection\textsubscript{G} & 1 & 1 & S \\
        \hline
        ...
        \hline
        test trasversali (smoke\textsubscript{G}, regressione Undo/Redo) & 3 & 32 & S \\
        \hline
        \textbf{Totale frontend} & \textbf{45} & \textbf{298} & \textbf{298/298 S} \\
        \hline
        \rowcolor{primarycolor!10}
        \textbf{Totale complessivo TU} & \textbf{57} & \textbf{356} & \textbf{356/356 S} \\
        \hline
    \end{tabularx}
    \caption{Test di Unità -- riepilogo per componente (esecuzione verificata alla data di redazione)}
\end{table}

Copertura di codice risultante: \textbf{95\%} sul backend (soglia CI 90\%) e \textbf{89,75\% linee / 88,51\% statement / 89,72\% funzioni / 83,61\% branch} sul frontend, calcolata sull'intera suite (TU + Test di Sistema, ora eseguiti insieme -- si veda la nota di integrazione sopra), a fronte di una soglia CI del 70\% complessivo e dell'85\%/78\%/78\% sulle soglie puntuali di R1-Q-O: entrambe verificate rieseguendo \texttt{make coverage} in sede di stesura del documento.
