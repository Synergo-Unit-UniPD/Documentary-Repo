...
I criteri operativi della Definition of Done sono definiti nelle \link{https://synergo-unit-unipd.github.io/Second-Brain/docs/PB/doc_gestionali/doc_interni/norme_di_progetto/norme_di_progetto.pdf}{Norme di
...
documentazione;
\item i criteri specifici per task di implementazione, nel caso di attività
di sviluppo del prodotto software.
\end{itemize}
...
I criteri per i task di implementazione sono aumentati passando dalla RTB\textsubscript{G}
alla PB\textsubscript{G}, coerentemente con la maturità crescente richiesta al
prodotto (Proof of Concept\textsubscript{G} in RTB, Minimum Viable
Product\textsubscript{G} in PB): la Sezione~\ref{sec:dod-task} descrive i
criteri correntemente in vigore.
...
Per i task di implementazione, il completamento richiede che il codice sia
coerente con lo scope della issue e con le Norme di Progetto (naming,
formattazione, organizzazione dei moduli), che non siano presenti errori
evidenti in console, credenziali, token o file sensibili nel
repository\textsubscript{G}, che eventuali modifiche rilevanti siano riflesse
nei documenti coinvolti (in particolare Specifica Tecnica e tracciamento
componenti--requisiti) e che siano stati scritti test automatizzati per la
funzionalità introdotta o modificata, almeno di unità e, quando la modifica
coinvolge più componenti architetturali, anche di integrazione o di sistema.
La pipeline di CI\textsubscript{G} (\texttt{make ci}: analisi statica, type
check, test e verifica delle soglie di copertura) deve risultare verde prima
del merge; la Pull Request deve inoltre aver ricevuto almeno una review
positiva da un Verificatore diverso dall'autore della modifica.
