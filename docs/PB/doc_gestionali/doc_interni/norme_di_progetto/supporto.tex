...
\paragraph{Pull Request per branch ci codice}
...
\begin{itemize}
    ...
    \item se si passsano i test in locale si fa il push e la PR in draft fa partire la CI
    \item terminato lo sviluppo e compilata la checklist, si mette in ready per la review
    \item il verificatore fa la review e approva solo se tutto è in verde nella pipeline
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Code Coverage:} È fissata una soglia minima di copertura dei test pari all'\textbf{80\%}.
\end{itemize}
Tali controlli sono automatizzati tramite pipeline di Continuous Integration. 
Per ottimizzare i tempi, la pipeline di Continuous Integration è configurata per attivarsi esclusivamente in seguito a modifiche all'interno delle directory del codice sorgente (es. \texttt{frontend/}, \texttt{backend/}), ignorando i commit puramente documentali.
...
\begin{itemize}
    ...
    \item la \textit{code coverage} del modulo modificato o aggiunto è $\ge$ 80\%;
