...
La codifica e test del software riguarda lo sviluppo delle unità software, dei database e delle relative procedure di test
...
\begin{itemize}
    ...
    \item \textbf{Adapter e Repository:} Suffisso \texttt{Adapter} o \texttt{Repository} in base al pattern (es. \texttt{OpenAIAdapter}, \texttt{NoteRepository}). Un'eccezione è il pattern \texttt{Proxy} per il frontend (es. \texttt{NoteServiceProxy}).
...
L'integrazione delle unità software avviene attraverso il modello di trunk-based development, nel quale le modifiche apportate ai vari moduli confluiscono nel branch, dove prima di fare la push verso il main vengono fatti i test in locale e solo una volta passati avviene il push e la PR in draft fa partire la CI; infine, a termine dello sviluppo, viene compilata la checklist e solo dopo avviene l'azzione di verifica del verificatore, che accettera solo se è tutto in verde nella pipeline. A partire dagli Sprint 12 e 13, viene avviato il setup dell'ambiente di Continuous Integration/Continuous Deployment (CI/CD) per automatizzare i processi di build e verifica.

\paragraph{Test di Integrazione (TI)}
L'efficacia dell'unione dei moduli è validata tramite appositi Test di Integrazione (TI) volti a controllare la comunicazione tra le unità (es. frontend e backend), la cui pianificazione di dettaglio è prevista per la fase di Product Baseline (PB).
...
\paragraph{Manuale Tecnico (README)}

La procedura di installazione e configurazione nell'ambiente target è documentata nel file README presente nella cartella poc/ del repository. Tale documento riporta i prerequisiti necessari, la procedura di avvio tramite Docker Compose e le modalità di verifica del corretto funzionamento dell'intero sistema su host locale.
