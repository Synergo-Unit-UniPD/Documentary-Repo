...
\begin{itemize}
	...
	\item \textbf{Dipendenza da API browser-specifiche}: le funzionalità di salvataggio e apertura locale della nota (R75-F-O, R76-F-O) si appoggiano alla \textit{File System Access API} del browser (\texttt{showSaveFilePicker}, \texttt{showOpenFilePicker}, Sezione~\ref{sec:arch-frontend}). Il supporto a questa API varia tra i browser elencati in R5-V-O: è nativo su Chrome ed Edge, mentre su Firefox e Safari il supporto è storicamente parziale o assente nelle rispettive versioni correnti. 
    
    %\textbf{Questo punto necessita di verifica esplicita da parte del gruppo} (es. tramite test cross-browser nel Piano di Qualifica) per accertare la piena compatibilità con R5-V-O e la coerenza con R13-Q-D (assenza di dipendenze da funzionalità non standard dei browser); in assenza di tale verifica, si raccomanda di documentarne l'esito nella prossima revisione di questo documento.
\end{itemize}
