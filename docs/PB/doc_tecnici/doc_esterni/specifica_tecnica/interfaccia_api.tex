...
\begin{tabular}{l l p{7cm}}
	...
	\multirow{3}{*}{\texttt{AIOperationRequest}}
	 & \texttt{type} & Tipo di operazione richiesta (es. \texttt{summarize}, \texttt{translate}, \texttt{rewrite}, \texttt{distant\_writing}, \texttt{hat\_analysis}), risolto dinamicamente da \texttt{AIOperationFactory}. Le sei prospettive dei Cappelli di De Bono condividono un unico \texttt{type} (\texttt{hat\_analysis}): il cappello specifico è indicato nel campo \texttt{params.hat\_type} (es. \texttt{white}, \texttt{red}), non nel \texttt{type} stesso. \\
	 & \texttt{text} & Testo selezionato dall'utente su cui operare (contesto). \\
	 & \texttt{params} & Parametri aggiuntivi specifici dell'operazione (es. \texttt{target\_language} per la traduzione, \texttt{hat\_type} per l'analisi critica, \texttt{user\_prompt} per il Distant Writing). \\
	\midrule
...
Questa struttura unificata consente di aggiungere nuove operazioni AI (rispettando l'Open Closed Principle\textsubscript{G}, Sezione~\ref{sec:design-pattern}) senza introdurre nuovi endpoint o nuovi contratti di comunicazione in quanto è sufficiente registrare una nuova strategia lato Backend.

\subsection{Gestione degli Errori e Fallback\textsubscript{G}}
