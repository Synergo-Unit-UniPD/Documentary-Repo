...
\begin{itemize}
	\item un \textbf{monolite unico indifferenziato}, perchè avrebbe accoppiato il deploy del Frontend (statico) a quello del Backend (con dipendenze verso servizi LLM esterni), impedendo aggiornamenti o riavvii indipendenti dei due componenti;
	\item una \textbf{scomposizione a microservizi} (es. un servizio per ciascuna operazione AI) perchè non sarebbe giustificata dalla scala del sistema, che espone un dominio applicativo singolo e un volume di richieste limitato, inoltre, introdurrebbe complessità di orchestrazione (service discovery, API gateway) senza benefici concreti, tanto più in assenza di persistenza (Sezione~2.4).
\end{itemize}
