...
\begin{tabularx}{\textwidth}{|l|X|}
	...
	\hline
	\textbf{\link{https://docs.docker.com/compose/}{Docker Compose}} & Orchestrazione dei container \texttt{frontend} e \texttt{backend} (Sezione~\ref{sec:arch-deployment}). Il server di sviluppo Vite del container \texttt{frontend} inoltra le richieste verso \texttt{/api} al container \texttt{backend} (\texttt{http://backend:8000}); non è presente un reverse proxy\textsubscript{G} dedicato (es. Nginx) in questa fase. \\
	\hline
	Git e GitHub & Versionamento del codice e gestione collaborativa del repository\textsubscript{G}, con tracciamento delle attività tramite issue\textsubscript{G} (requisito R5-Q-O). \\
	\hline
	GitHub Actions\textsubscript{G} & Automazione di build, esecuzione dei test e controlli di qualità a ogni push/pull request\textsubscript{G} (requisiti R1-Q-O, R6-Q-O). \\
	\hline
